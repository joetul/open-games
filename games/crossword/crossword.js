...
let gridSize = 15;
let solutionGrid = [];   // 2D: solutionGrid[r][c] = 'A'-'Z' or '#'
...
function loadPuzzle(puzzle) {
  ...
  saveToStorage('crossword-last-id', puzzle.id);
}
...
function saveProgress() {
  ...
  saveToStorage(`crossword-progress-${currentPuzzle.id}`, {
    playerGrid,
    revealedCells,
    timerSeconds,
    selectedCell,
...
async function init() {
  ...
  try {
    const lastId = loadFromStorage('crossword-last-id');
    let puzzle = null;
